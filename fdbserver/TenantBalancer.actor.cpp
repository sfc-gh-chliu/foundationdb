...
ACTOR Future<Void> getActiveMovements(TenantBalancer* self, GetActiveMovementsRequest req) {
	...
	try {
		...
		for (int i = 0; i < filteredMovements.size(); ++i) {
			Optional<std::pair<MovementState, std::string>> newMovementState;
			const auto& curBackupState = backupStateFutures[i].get();
			if (curBackupState.isError()) {
				TraceEvent(SevWarn, "TenantBalancerFetchBackupStateError", self->tbi.id())
				    .error(curBackupState.getError())
...
ACTOR Future<Void> getStatusAndUpdateMovementRecord(TenantBalancer* self,
                                                    Reference<MovementRecord> record,
	...
	if (record->getMovementLocation() == MovementLocation::SOURCE) {
		DatabaseBackupStatus s =
		    wait(timeoutError(self->agent.getStatusData(record->getPeerDatabase(), 1, record->getTagName()),
		                      SERVER_KNOBS->TENANT_BALANCER_OPERATION_TIMEOUT));
		drStatus = s;
	} else {
		state DatabaseBackupAgent sourceAgent(record->getPeerDatabase());
		DatabaseBackupStatus s = wait(timeoutError(sourceAgent.getStatusData(self->db, 1, record->getTagName()),
		                                           SERVER_KNOBS->TENANT_BALANCER_OPERATION_TIMEOUT));
		drStatus = s;
	}
	...
	if (record->movementState == MovementState::STARTED || record->movementState == MovementState::READY_FOR_SWITCH ||
		...
		double versionLag =
		    wait(timeoutError(getDatabaseVersionLag(self, record), SERVER_KNOBS->TENANT_BALANCER_OPERATION_TIMEOUT));
...
ACTOR Future<Void> getMovementStatus(TenantBalancer* self, GetMovementStatusRequest req) {
	...
	state std::string potentialTimeoutFunction;
	try {
		...
		potentialTimeoutFunction = "getStatusAndUpdateMovementRecord";
		wait(timeoutError(getStatusAndUpdateMovementRecord(self, record, &status),
		                  SERVER_KNOBS->TENANT_BALANCER_OPERATION_TIMEOUT));

		potentialTimeoutFunction = "saveMovementRecord";
		if (mutableRecord.present()) {
			ErrorOr<Void> saveRecordResult = wait(errorOr(timeoutError(
			    self->saveMovementRecord(mutableRecord.get()), SERVER_KNOBS->TENANT_BALANCER_OPERATION_TIMEOUT)));
			if (saveRecordResult.isError()) {
				TraceEvent(SevWarn, "TenantBalancerSaveMovementRecordError", self->tbi.id())
				    .error(saveRecordResult.getError())
				    .detail("MovementId", record->getMovementId());
			}
		}
		...
		if (e.code() == error_code_timed_out) {
			...
			TraceEvent(SevWarn, "TenantBalancerOperationTimeout", self->tbi.id())
			    ...
			    .detail("TimeoutFunction", potentialTimeoutFunction)
			    .detail("Prefix", req.prefix)
			    .detail("MovementLocation", TenantBalancerInterface::movementLocationToString(req.movementLocation));
			GetMovementStatusReply reply(status);
			reply.movementStatus.tenantMovementInfo.error = e;
			req.reply.send(reply);
		} else {
...
ACTOR Future<Void> sendFinishRequestToDestination(TenantBalancer* self,
                                                  ...
                                                  std::string* potentialTimeoutFunctionPtr = nullptr) {
	state Reference<MovementRecord> record = mutableRecord->record;
	Future<FinishDestinationMovementReply> finishDestinationMovementEvent = waitOrError(
	    ...
	    record->onAbort());
	if (potentialTimeoutFunctionPtr != nullptr) {
		*potentialTimeoutFunctionPtr = "sendTenantBalancerRequest";
	}
	FinishDestinationMovementReply destinationReply = wait(timeoutError(
	    finishDestinationMovementEvent,
	    maxLagSeconds.present() ? maxLagSeconds.get() + 5 * SERVER_KNOBS->TENANT_BALANCER_OPERATION_TIMEOUT : DBL_MAX));

	if (potentialTimeoutFunctionPtr != nullptr) {
		*potentialTimeoutFunctionPtr = "saveMovementRecord(MovementState::COMPLETED)";
	}
	...
	if (potentialTimeoutFunctionPtr != nullptr) {
		*potentialTimeoutFunctionPtr = "abortPeer";
	}
...
ACTOR Future<bool> isPeerAlive(TenantBalancer* self, Reference<MovementRecord> record) {
	...
	try {
		peerStatusReply = wait(timeoutError(
		    ...
		    SERVER_KNOBS->TENANT_BALANCER_OPERATION_TIMEOUT));
		if (peerStatusReply.movementStatus.tenantMovementInfo.movementId != record->getMovementId()) {
			return false;
		}
	} catch (Error& e) {
		TraceEvent(SevDebug, "TenantBalancerIsPeerAlive", self->tbi.id())
		    .error(e)
		    .detail("PeerPrefix", record->getRemotePrefix())
		    .detail("PeerLocation",
		            TenantBalancerInterface::movementLocationToString(
		                record->getMovementLocation() == MovementLocation::SOURCE ? MovementLocation::DEST
		                                                                          : MovementLocation::SOURCE));
		return false;
	}
...
ACTOR Future<Void> finishSourceMovement(TenantBalancer* self, FinishSourceMovementRequest req) {
	...
	state std::string potentialTimeoutFunction;
	try {
		potentialTimeoutFunction = "mutateOutgoingMovement";
		...
		potentialTimeoutFunction = "isPeerAlive";
		...
		potentialTimeoutFunction = "getStatusAndUpdateMovementRecord";
		wait(timeoutError(
		    waitOrError(getStatusAndUpdateMovementRecord(self, record, &movementStatus), record->onAbort()),
		    SERVER_KNOBS->TENANT_BALANCER_OPERATION_TIMEOUT));
		...
		if (!movementStatus.mutationLag.present() ||
		    (req.maxLagSeconds.present() && movementStatus.mutationLag.get() > req.maxLagSeconds.get())) {
		...
		potentialTimeoutFunction = "lockSourceTenant";
		...
		TraceEvent(SevDebug, "TenantBalancerFinishSourceMovementError", self->tbi.id())
		    ...
		    .detail("SourcePrefix", req.sourcePrefix)
		    .detail("MaxLagSeconds", req.maxLagSeconds)
		    .detail("LastTimeoutFunction", potentialTimeoutFunction);
		req.reply.sendError(e);
	}
...
ACTOR Future<Void> finishDestinationMovement(TenantBalancer* self, FinishDestinationMovementRequest req) {
	...
	state std::string potentialTimeoutFunction;
	try {
		potentialTimeoutFunction = "mutateIncomingMovement";
		state Reference<MovementRecordMap::MutableRecord> mutableRecord =
		...
		if (record->movementState == MovementState::STARTED) {
			potentialTimeoutFunction = "getDrState";
		...
		if (record->movementState == MovementState::SWITCHING) {
			...
			potentialTimeoutFunction = "flushBackup";
			wait(timeoutError(
			    waitOrError(sourceBackupAgent.flushBackup(self->db, record->getTagName(), record->switchVersion),
			                record->onAbort()),
			    req.maxLagSeconds.orDefault(DBL_MAX)));
			// TODO: unlock DR prefix

			record->movementState = MovementState::COMPLETED;
			potentialTimeoutFunction = "saveMovementRecord(MovementState::COMPLETED)";
			wait(timeoutError(waitOrError(self->saveMovementRecord(mutableRecord), record->onAbort()),
			                  SERVER_KNOBS->TENANT_BALANCER_OPERATION_TIMEOUT));
		}
...
ACTOR Future<Void> clearTenant(TenantBalancer* self,
                               ...
                               bool doErase,
                               bool doUnlock,
                               std::string* potentialTimeoutFunctionPtr) {
	state Reference<MovementRecord> record = mutableRecord->record;
	...
	if (doErase) {
		...
		*potentialTimeoutFunctionPtr = "runTenantBalancerTransaction";
	...
	if (doUnlock) {
		...
		*potentialTimeoutFunctionPtr = "clearMovementRecord";
...
ACTOR Future<Void> rollbackMovement(TenantBalancer* self,
	...
	*potentialTimeoutFunctionPtr = "timeoutError";
	...
	wait(clearTenant(self, mutableRecord, MovementLocation::DEST, eraseData, true, potentialTimeoutFunctionPtr));
	return Void();
}
...
ACTOR Future<Void> abortMovement(TenantBalancer* self, AbortMovementRequest req) {
	...
	try {
		...
		potentialTimeoutFunction = "mutateMovement";
		state Reference<MovementRecordMap::MutableRecord> mutableRecord =
		...
		if (record->getMovementLocation() == MovementLocation::SOURCE) {
			if (record->movementState == MovementState::COMPLETED) {
				...
				if (req.abortInstruction == AbortState::COMPLETED) {
					...
					wait(rollbackMovement(self, mutableRecord, false, &potentialTimeoutFunction));
					abortResult = AbortState::ROLLED_BACK;
				} else {
					// There isn't any previous destination abort
					potentialTimeoutFunction = "abortDr";
					ErrorOr<Void> result =
					    wait(timeoutError(abortDr(self, record), SERVER_KNOBS->TENANT_BALANCER_OPERATION_TIMEOUT));
					if (result.isError() && result.getError().code() != error_code_backup_unneeded) {
						throw result.getError();
			...
			if (record->movementState == MovementState::COMPLETED) {
				...
				TraceEvent(SevDebug, "TenantBalancerAbortCompletedMovement", self->tbi.id())
				    .detail("MovementId", req.movementId)
				    .detail("Prefix", req.prefix);
				wait(clearTenant(self, mutableRecord, MovementLocation::DEST, false, true, &potentialTimeoutFunction));
				abortResult = AbortState::COMPLETED;
			} else {
				if (req.abortInstruction == AbortState::COMPLETED) {
					throw movement_abort_error();
				}
				wait(rollbackMovement(self, mutableRecord, true, &potentialTimeoutFunction));
				abortResult = AbortState::ROLLED_BACK;
			}
		}
		...
		TraceEvent(SevDebug, "TenantBalancerAbortError", self->tbi.id())
		    .error(e)
		    .detail("ErrorFunction", potentialTimeoutFunction)
		    .detail("MovementId", req.movementId)
		    .detail("MovementLocation", TenantBalancerInterface::movementLocationToString(req.movementLocation))
		    .detail("Prefix", req.prefix);
