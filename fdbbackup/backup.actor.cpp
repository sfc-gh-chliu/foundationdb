...
ACTOR Future<Void> submitDBMove(Database src, Database dest, Key srcPrefix, Key destPrefix) {
	try {
		...
		loop choose {
			when(ErrorOr<MoveTenantToClusterReply> reply = wait(timeoutError(replyFuture, requestTimeout))) {
				if (reply.isError()) {
...
ACTOR Future<TenantMovementStatus> getMovementStatus(Database database, Key prefix, MovementLocation movementLocation) {
	...
	state Future<Void> initialize = Void();
	loop choose {
		when(ErrorOr<GetMovementStatusReply> reply = wait(timeoutError(getMovementStatusReply, requestTimeout))) {
			if (reply.isError()) {
				throw reply.getError();
			}
...
ACTOR Future<std::vector<TenantMovementInfo>> getActiveMovements(
	...
	state Future<Void> initialize = Void();
	loop choose {
		when(ErrorOr<GetActiveMovementsReply> reply = wait(timeoutError(getActiveMovementsReply, requestTimeout))) {
			if (reply.isError()) {
				throw reply.getError();
			}
...
ACTOR Future<Void> listDBMove(Database db, bool isSrc) {
	...
	wait(fetchAndDisplayDBMove(db, Optional<std::string>(), locationFilter));
	return Void();
}

// list movement from src to dest
ACTOR Future<Void> listDBMove(Database src, Database dest) {
	std::string destConnectionString = dest->getConnectionRecord()->getConnectionString().toString();
	wait(fetchAndDisplayDBMove(src, destConnectionString, MovementLocation::SOURCE));
	return Void();
}
	...
	try {
		...
		loop choose {
			when(ErrorOr<FinishSourceMovementReply> reply =
			         wait(timeoutError(finishSourceMovementReply, requestTimeout))) {
				if (reply.isError()) {
					throw reply.getError();
				}
				break;
...
ACTOR Future<AbortState> abortDBMove(Database database,
	...
	state Future<Void> initialize = Void();
	loop choose {
		when(ErrorOr<AbortMovementReply> reply = wait(timeoutError(abortMovementReply, requestTimeout))) {
			if (reply.isError()) {
				throw reply.getError();
			}
...
ACTOR Future<Void> abortDBMove(Optional<Database> src,
	...
	try {
		// Check parameters are valid and get movementId
		state std::vector<Future<ErrorOr<TenantMovementStatus>>> movementStatuses;
		if (src.present()) {
			ASSERT(sourcePrefix.present());
			movementStatuses.push_back(
			    errorOr(getMovementStatus(src.get(), sourcePrefix.get(), MovementLocation::SOURCE)));
		}
		if (dest.present()) {
			ASSERT(destinationPrefix.present());
			movementStatuses.push_back(
			    errorOr(getMovementStatus(dest.get(), destinationPrefix.get(), MovementLocation::DEST)));
		}
		...
		// Abort the destination movement
		state AbortState destAbortResult = AbortState::UNKNOWN;
		if (dest.present()) {
			ErrorOr<AbortState> tempDestAbortResult = wait(errorOr(abortDBMove(
			    dest.get(), destinationPrefix.get(), MovementLocation::DEST, movementId, abortInstruction)));
			if (tempDestAbortResult.isError()) {
				...
				destAbortResult = tempDestAbortResult.get();
			}
			if (!src.present()) {
	...
	try {
		...
		loop choose {
			when(ErrorOr<CleanupMovementSourceReply> reply =
			         wait(timeoutError(cleanupMovementSourceReply, requestTimeout))) {
				if (reply.isError()) {
					throw reply.getError();
				}
				break;
